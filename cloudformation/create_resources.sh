...
aws cloudformation deploy \
    ...
    --parameter-overrides DatabaseId=primary Subnet1=$PRIMARY_PRIVATE_SUBNET1_ID Subnet2=$PRIMARY_PRIVATE_SUBNET2_ID DatabaseSecurityGroup=$PRIMARY_DB_SG DBUsername=root DBPassword=testtest1 SourceDBId='' SourceDBRegion='' \
    --profile udacity \
    --region us-west-2

PRIMARY_DB_ID=`aws cloudformation describe-stacks --stack-name PrimaryRDS --query "Stacks[0].Outputs[?OutputKey=='DatabaseId'].OutputValue" --output text --profile udacity --region us-west-2`
PRIMARY_DB_ARN=`aws rds describe-db-instances --db-instance-identifier $PRIMARY_DB_ID --query "DBInstances[0].DBInstanceArn" --profile udacity --region us-west-2 --output text`

aws cloudformation deploy \
    --stack-name SecondaryRDS \
    --template-file ./rds.yaml \
    --parameter-overrides DatabaseId=secondary Subnet1=$SECONDARY_PRIVATE_SUBNET1_ID Subnet2=$SECONDARY_PRIVATE_SUBNET2_ID DatabaseSecurityGroup=$SECONDARY_DB_SG DBUsername=root DBPassword=testtest1 SourceDBId=$PRIMARY_DB_ARN SourceDBRegion=us-west-2 \
    --profile udacity \
    --region us-east-1
...
aws cloudformation deploy \
    ...
    --template-file ./ec2.yaml \
    --parameter-overrides ApplicationSecurityGroup=$SECONDARY_APP_SG SSHKeyName=udacity2-ssh InstanceImageId=$SECONDARY_IMAGE_ID SubnetId=$SECONDARY_PUBLIC_SUBNET1_ID \
    --profile udacity \
    --region us-east-1